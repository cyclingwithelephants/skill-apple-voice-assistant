...
- **User confirmation requirement.** The user requires an audit message every time a voice memo is processed, regardless of category or whether the action succeeded, failed, or only created a draft. Send it to the configured audit target (`APPLE_VOICE_ASSISTANT_AUDIT_TARGET` env var — the webhook subscription's `--deliver-chat-id` sets this). If delivery fails, append a FOLLOW-UP line to `~/.local/state/apple-voice-assistant/TODO.md` with enough detail to replay the missed confirmation later.
...
- For audit delivery, use the target specified by `APPLE_VOICE_ASSISTANT_AUDIT_TARGET`. The webhook subscription configures delivery automatically; action files reference this env var.
